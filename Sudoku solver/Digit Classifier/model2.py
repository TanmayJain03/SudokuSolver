...
class Trainer:
	def __init__(self):
		...
		self.num_epochs = 75 # Number of Epochs to train for
	...
	def load_data(self):
		...
		self.loader.train_data = self.loader.train_data.reshape((-1,784))
		self.loader.train_labels = self.loader.train_labels.reshape((-1,1))
		self.loader.test_data = self.loader.test_data.reshape((-1,784))
	...
	def save_model(self):
		# Save the model parameters into the file 'assets/model2'
		# eg. For pytorch, torch.save(self.model.state_dict(), 'assets/model2')
		# Start Editing
		self.model.save('assets/model2')
	...
	def load_model(self):
		# Load the model parameters from the file 'assets/model2'
		if os.path.exists('assets/model2'):
			# eg. For pytorch, self.model.load_state_dict(torch.load('assets/model2'))
			self.model = keras.models.load_model('assets/model2')
			pass
		else:
